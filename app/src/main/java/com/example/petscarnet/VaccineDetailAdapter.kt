...
class VaccineDetailAdapter (var mCtx: Context,
                            val resources: Int,
                            val items:List<Vaccine>): ArrayAdapter<Vaccine>(mCtx,resources, items){
    ...
    override fun getView(position:Int, convertView: View?, parent: ViewGroup): View {
        ...
        var mItem: Vaccine = items[position]

        val pattern = "yyyy-MM-dd"
        val simpleDateFormat = SimpleDateFormat(pattern)

        titleTextView.text = mItem.name
        descTextView.text = simpleDateFormat.format(mItem.applicationDate)
